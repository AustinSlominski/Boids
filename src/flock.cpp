...
void Boid::update()
{
    if(b_color){
        ...
        //---------CIRCULAR BUFFER-------------
        //-------------------------------------
        accumulator[curr_ind] = ofVec3f(ali.lengthSquared(),sep.lengthSquared(),coh.lengthSquared()) * velocity.lengthSquared();
        if(curr_ind % ARR_LEN){
            ...
            running_avg = previous_avg.interpolate(running_avg,((float)curr_ind/(float)ARR_LEN));
        }
